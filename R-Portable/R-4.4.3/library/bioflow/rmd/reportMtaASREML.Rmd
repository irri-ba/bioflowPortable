...
if (is.null(result)){
  ...
  if("GenCorrMat" %in% names(predSplit)){predSplit[["GenCorrMat"]]<-NULL}
  ...
  traits2 <- unique(result$metadata$pheno[which(result$metadata$pheno$parameter == "trait"), "value"])
    gxeList <- list("gxeCS" = c("environment_designation", "designation_environment", 
                  ...
                  "gxeDMD" = c(paste0("env",envs2,"_designation"), paste0("designation_env",envs2),
                               paste0("env",envs2,":designation"), paste0("designation:env",envs2)),
  ...
  if(gxeModelNum == 2){
    effectTypes2 <- setdiff(effectTypes, c(effectTypes[which(effectTypes %in% gxeList[[2]])],
                                           effectTypes[which(effectTypes %in% paste0("env",envs2))]))
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    inputPanel(
      shiny::selectInput(ns("traitMtaConnect"), 
                  label = "Trait", 
                  choices = traits
      ),
      column(width=12),
    ...
    for(iTrait in traits){
      ...
      if(length(goodFields) > 0){
        ...
        p2 <- ggplot(mydataSta4[which(mydataSta4$Var1 != mydataSta4$Var2),], aes(x=Freq, fill=Group)) + 
          ...
          scale_fill_manual(values=group.colors) + ggplot2::xlab("Frequency") + ggplot2::ylab("")
      }else{
    ...
    plotly::renderPlotly({ # dynamic
      ...
      if(length(goodFields) > 0){
        ...
        if(input$freqConnect == TRUE){
          ...
          plotly::subplot(ply1, ply2, nrows=1, shareY = TRUE, shareX = TRUE) %>% layout(autosize = T, width = 1600)
        }else{
          plotly::ggplotly(p  + geom_text(aes(label=Freq))) %>% layout(xaxis = list(tickangle =-45),autosize = T,height = 800)
        }#end input$freqConnect
    })#end plotly
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    if(any(c("environment_designation", "designation_environment",
      ...
      inputPanel(
        shiny::selectInput(ns("traitPredictionsCorrelation"),
                    label = "Trait",
                    choices = traits
        ),
        column(width=12),
...
if (is.null(result)){
  # invisible(NULL)
   ...
   predGenCorrMat <- split(pred, pred[,"effectType"])
  PreCalcMat<-FALSE
  if("GenCorrMat" %in% names(predGenCorrMat)){predGenCorrMat<-predGenCorrMat[["GenCorrMat"]];PreCalcMat<-TRUE}
 if(gxeModelNum != 0){
        ...
        if(!is.null(wide2)){
          if(nrow(wide2) > 1){
			if(PreCalcMat==FALSE){
				...
				wideG <- predGenCorrMat[which(predGenCorrMat$trait==iTrait2),]  
				levels_all <- sort(unique(c(wideG$designation, wideG$environment)))
        all_pairs <- expand.grid(designation = levels_all,environment = levels_all,stringsAsFactors = FALSE)
        df_full <- merge(all_pairs, wideG, by = c("designation","environment"), all.x = TRUE)
        df_full$predictedValue[is.na(df_full$predictedValue)] <- 0
				corr <- reshape2::acast(df_full, designation ~ environment, value.var = "predictedValue")
				corr <- corr + t(corr)
				corr <- round(corr,2)
				diag(corr) <- 1 
				namePlot<-"Genetic\nCorrelation"
      ...
      plotly::renderPlotly({
        ...
        if(length(level3)>0){
		  ...
		  if(PreCalcMat==FALSE){
				...
				wideG <- predGenCorrMat[which(predGenCorrMat$trait==input$traitPredictionsCorrelation),]  
				levels_all <- sort(unique(c(wideG$designation, wideG$environment)))
        all_pairs <- expand.grid(designation = levels_all,environment = levels_all,stringsAsFactors = FALSE)
        df_full <- merge(all_pairs, wideG, by = c("designation","environment"), all.x = TRUE)
        df_full$predictedValue[is.na(df_full$predictedValue)] <- 0
        ...
        if(input$freqCor == TRUE){
          ...
          if(length(level3)>0){
            plotly::ggplotly(p + geom_text(aes(label=Freq), size = 3)) %>% layout(xaxis = list(tickangle =-45),autosize = T,height = 800)
          } else{
            plotly::ggplotly(p)
          }
          ...
          if( all(c( ncol(pred.mta.biplot) >1, is.matrix(pred.mta.biplot)  ) ) ){
            ...
            for (i in 1:ncol(pred.mta.biplot)){
              p.mta.biplot <- p.mta.biplot %>%
                plotly::add_segments(x = 0, xend = loadings[i, 1], y = 0, yend = loadings[i, 2], line = list(color = 'red'),inherit = FALSE, showlegend = FALSE, text=rownames(loadings)[i],hoverinfo = "text")
    ...
    if(any(effectTypes %in% gxeList[[1]])){
      ...
      if("params" %in% ls()){ # we will download the document
        ...
        plotly::renderPlotly({
          ...
          plotly::layout(p.mta.biplot) #%>% layout(autosize = T, width = 400)
        }) # end of plotly
      }
    } else{
...
if (is.null(result)){
  ...
  if("params" %in% ls()){ # we will download the document
    ...
    if(length(traits) > 1){
      inputPanel(
        shiny::checkboxInput(ns("freqCorT"),
                             label = "Show frequency distribution",
                             value = TRUE
        )
      )
